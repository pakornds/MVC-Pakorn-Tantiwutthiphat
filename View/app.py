...
class Window:
    def __init__(self, root):
        ...
        # Set the result_text to be read-only
    ...
    def submit_cow_id(self):
        ...
        if not cow_id.isdigit() or len(cow_id) != 8 or cow_id[0] == '0':
            ...
            return
        # Submit cow ID to the controller
        self.controller.submit_cow_id(cow_id)

    def display_message(self, message):
        """Displays a message in the result text area."""
        # Temporarily set state to NORMAL to update content
        self.result_text.config(state=tk.NORMAL)
        self.result_text.delete(1.0, tk.END)  # Clear existing content
        self.result_text.insert(tk.END, message)  # Insert new message
        self.result_text.config(state=tk.DISABLED)  # Re-enable read-only
